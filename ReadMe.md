...
Good unit tests are repeatable, they always produce the same results from a fixed set of inputs.  Intuitively, this makes a lot of sense but it can be challenging to achieve in real practice: for example programming logic that depends on the current date or time might give different results as the current date or time changes.  That might be desirable for a software program, however it is not so for testing it; tests need to be consistent and therefore can't depend on the current date or time.
...
Regarding randomization, the `Determination` library brings in other benefits in addition to improved testability:  it allows flexibility in choosing how the randomization is implemented.  For example `.NET` offers a couple of mechanisms for randomization:  There is the [System.Random](https://docs.microsoft.com/en-us/dotnet/api/system.random?view=netcore-3.1) class and also the [System.Security.Cryptography.RNGCryptoServiceProvider](https://docs.microsoft.com/en-us/dotnet/api/system.security.cryptography.rngcryptoserviceprovider?view=netcore-3.1) class.  In a production system either one of those classes, or perhaps something else might be used to generate random values; for testing a user may chose to provide some hard-coded values instead.
