...
In addition, it is also possible to extend the library by creating other descendents of the interface `IDynamicValueProvider<T>` and creating implementations of the new interface in a similar manner to how the `DateTimeProvider`/`DateTimeProviderStub` and `GuidProvider`/`GuidProviderStub` classes do it.
